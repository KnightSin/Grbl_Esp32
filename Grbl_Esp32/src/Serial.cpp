    ...
    while (true) {  // run continuously
        while (any_client_has_data()) {
            if (Serial.available()) {
                ...
                data = 0;
                while (WebUI::inputBuffer.available() && data != '\n') {
                    client = CLIENT_INPUT;
                ...
                if (WebUI::SerialBT.hasClient() && WebUI::SerialBT.available()) {
                    data = 0;
                    while (WebUI::SerialBT.available() && data != '\n') {
                        client = CLIENT_BT;
                    ...
                    if (WebUI::Serial2Socket.available()) {
                        data = 0;
                        while (WebUI::Serial2Socket.available() && data != '\n') {
                            client = CLIENT_WEBUI;
                        ...
                        if (WebUI::telnet_server.available()) {
                            data = 0;
                            while (WebUI::telnet_server.available() && data != '\n') {
